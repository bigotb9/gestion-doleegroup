  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        ...
        const cp = data.conditionsPaiement ?? ""
        try {
          const parsed = JSON.parse(cp)
          if (typeof parsed.commande === "number" && typeof parsed.livraison === "number") {
            setConditionsMode("structured")
            setConditionsCommande(parsed.commande)
          } else { throw new Error() }
        } catch {
          setConditionsMode(cp ? "free" : "structured")
          setConditionsTexte(cp)
        }
